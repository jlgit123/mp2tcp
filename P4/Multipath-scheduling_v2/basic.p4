...
control MyIngress(inout headers hdr,
    ...
    action threshold_path1(){
        ...
        threshold.read(meta.threshold3,(bit<32>)2);
    }
    action threshold_path2(){
        threshold.read(meta.threshold1,(bit<32>)3);
        threshold.read(meta.threshold2,(bit<32>)4);
        threshold.read(meta.threshold3,(bit<32>)5);
    }
    action threshold_path3(){
        threshold.read(meta.threshold1,(bit<32>)6);
        threshold.read(meta.threshold2,(bit<32>)7);
        threshold.read(meta.threshold3,(bit<32>)8);
    }
    action drop() {
        mark_to_drop();
    }
   ...
   table ipv4_lpm {
	...
	if (standard_metadata.ingress_port == 1&&hdr.ipv4.isValid()){
	    ...
	    if(meta.hashcode < 85){
		...
		else if(meta.tempcount < meta.threshold2){
                    meta.tempport = 2;
                    //ipv4_lpm2.apply();
		    count.write((bit<32>)meta.hashcode,meta.tempcount+1);
                }
		    ...
		    count.write((bit<32>)meta.hashcode,meta.tempcount+1);
                ...
                if(meta.tempcount == meta.threshold3-1){
                    count.write((bit<32>)meta.hashcode,(bit<32>)0);
                }
	    }
	    else if(meta.hashcode < 170){
		threshold_path2();
                if(meta.tempcount < meta.threshold1){
                    meta.tempport = 2;
                    //ipv4_lpm2.apply();
	    ...
	    //if(meta.hashcode >= 170 && meta.hashcode <= 255){
            else{
		threshold_path3();
                if(meta.tempcount < meta.threshold1){
                    meta.tempport = 3;
