...
#
# author:jin 
# summary:获取联通网卡的ipv4,移动和电信的ipv6,转换为十进制标准格式,每1秒检测一次，
#         仅在变化时打印并写入文件s1_dynamic.txt,simple_switch_cli读取文件,无需打印可自行注释。
# time ：2019/3/21
# modified-1 by :  3/25   
# modify-1-log：查看具体namespace下网卡;匹配方式由位置切片匹配更改为字符匹配(避免地址出现不全的状况);完善注释和代码规范
# modified-2 by :  -3/29
# modify-2-log：对于出现3个ipv6地址的情况进行处理，更改匹配字符为Scope:Global筛选出两个，再根据与mac地址的后16位与ipv6地址后16位不同筛选出一个唯一的ipv6
...
flag_id0 = 'wwp0s20f0u1u2i5'  #网卡名字--联通 ns1 
flag_id1 = 'wwp0s20f0u1u3i5'  #移动 ns2
flag_id2 = 'wwp0s20f0u1u4i5'  #电信 ns3
command_0 = 'ifconfig'
...
ipv6_flag = 'Scope:Global'  #更改匹配字段为Scope --3/29
ipv6_1 = ''
ipv6_temp_1 = ''
mac_1 = ''
...
def get_ipv4_0():
   ...
   # p = Popen(['ip netns exec ns0 ifconfig'], stdout = PIPE) 
...
def get_ipv6_1():
    ...
    mac_1_str = mac_1.split(':')[-2:] 
    mac_1_str = ''.join(mac_1_str) 	     #获取mac后16位
    for i in range(0,len(list_1)-1):
        if list_1[i].find(ipv6_flag) != -1:    #一次筛选ipv6地址--Global
            list_2 = list_1[i].split()
            ipv6_1_str = list_2[2].split('/')[0].split(':')[-1]
            if ipv6_1_str != mac_1_str:  #二次筛选--后16位地址--3/29
                ipv6_temp_1 = list_2[2]   
                global ipv6_1   			
...
def get_ipv6_2():
    ...
    mac_2_str = mac_2.split(':')[-2:] 
    mac_2_str = ''.join(mac_2_str) 	     #获取mac后16位    
    for i in range(0,len(list_1)-1):
        if list_1[i].find(ipv6_flag) != -1:  #一次筛选ipv6地址
            list_2 = list_1[i].split()
            ipv6_2_str = list_2[2].split('/')[0].split(':')[-1]
            if ipv6_2_str != mac_2_str:  #二次筛选--3/29
                ipv6_temp_2 = list_2[2]  
                global ipv6_2   			
